...
def _info_command(args):
    try:
        ...
        output += f"Bid: {info['bid']} {info['currency']}\nBid Size: {info['bidSize']}\nAsk: {info['ask']} {info['currency']}\nAsk Size: {info['askSize']}\n\n"
        output += f"Price In Period {period} {interval}:\nPrevious Price: {info['previous_price']} {info['currency']}\nCurrent Price: {info['current_price']} {info['currency']}\nPrice Change: {info['price_change']} {info['currency']} ({info['price_change_percent']}%)\nMin Price in Period: {info['min_price_in_period']} {info['currency']}\nMax Price in Period: {info['max_price_in_period']} {info['currency']}\nAverage Price in Period: {info['avg_price_in_period']} {info['currency']}\n\n"
        output += f"Price Statistics\n50 Day Average: {info['50DayAverage']} {info['currency']}\n200 Day Average: {info['200DayAverage']} {info['currency']}\n52 Week Low: {info['52wkLow']} {info['currency']}\n52 Week High: {info['52wkHigh']} {info['currency']}\nAll Time High: {info['allTimeHigh']} {info['currency']}\nAll Time Low: {info['allTimeLow']} {info['currency']}\nTarget High Price: {info['targetHighPrice']} {info['currency']}\nTarget Low Price: {info['targetLowPrice']} {info['currency']}\nTarget Mean Price: {info['targetMeanPrice']} {info['currency']}\nTarget Median Price: {info['targetMedianPrice']} {info['currency']}"
        TelegramService.send_message(current_app, output)
    except Exception as e:
...
def _alert_unset_command(args):
    if not args or len(args) < 1:
        TelegramService.send_message(current_app, "Missing parameters. Please provide an alert id e.g. /alert_unset 5")
        return
    ...
    try:
        ...
        TelegramService.send_message(current_app, f"Alert with id {alert_id} deleted successfully.")
    except LookupError as le:
        logger.error(f"Error deleting alert: {le}")
        TelegramService.send_message(current_app, f"Error deleting alert: {le}")
    except Exception as e:
